...
%
f0 = readNumbersFromFile("segment_to_analyze.PitchTier");
...
for i = 1:num_p_values
    ...
    for j = 1:num_frames
        ...
        % Extract the excitation signal (train pulses when voiced, noise
        % otherwise):
        excitation_sinal_windowed = excitation_signal(start_idx:end_idx);

        % Re-synthesize using the LPC coefficients and error signal
        reconstructed_frame = filter(1, lpc_coeffs(:, j), excitation_sinal_windowed);
